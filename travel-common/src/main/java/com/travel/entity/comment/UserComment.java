...
public class UserComment implements Serializable{
	 ...
	 * 
	 */
	private static final long serialVersionUID = 9071855425918478449L;
	@Id
	...
	public UserComment myclone(UserComment uc){
		UserComment u = new UserComment();
		u.setAbsTags(uc.getAbsTags());
		u.setBrandList(uc.getBrandList());
		u.setBrandsTags(uc.getBrandsTags());
		u.setCommentEnvriomentScore(uc.getCommentEnvriomentScore());
		u.setCommentId(uc.getCommentId());
		u.setCommentPics(uc.getCommentPics());
		u.setCommentProdScore(uc.getCommentProdScore());
		u.setComments(uc.getComments());
		u.setCommentServiceScore(uc.getCommentServiceScore());
		u.setCommentText(uc.getCommentText());
		u.setCommentTime(uc.getCommentTime());
		u.setCommentTitle(uc.getCommentTitle());
		u.setDataLang(uc.getDataLang());
		u.setDetail(uc.getDetail());
		u.setLargeCommentPics(uc.getLargeCommentPics());
		u.setLocation(uc.getLocation());
		u.setPoUserPic(uc.getPoUserPic());
		u.setRawId(uc.getRawId());
		u.setStarLevel(uc.getStarLevel());
		u.setStarTag(uc.getStarTag());
		u.setTagList(uc.getTagList());
		u.setUpdateTs(new Date());
		u.setUserId(uc.getUserId());
		u.setUserName(uc.getUserName());
		u.setUserPic(uc.getUserPic());
		return u;
	}
	public List<Tag> getTagFromJson(){
		if(TravelUtils.isEmpty(absTags))return null;
		Gson gson = new Gson();
		List<Tag> tags = gson.fromJson(absTags, new TypeToken<List<Tag>>(){}.getType());
		return tags;
	}
}
